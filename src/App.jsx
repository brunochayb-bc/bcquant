      ...
      <div className="max-w-sm w-full">
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-3">
              <label className="block text-[10px] font-mono text-zinc-500 uppercase tracking-wider mb-1.5">E-mail</label>
              <input
                type="email"
                autoComplete="email"
                value={email}
                ...
                disabled={loading}
                className="w-full px-3 py-2.5 text-sm font-mono bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-100 placeholder-zinc-600 focus:border-emerald-500/50 focus:outline-none disabled:opacity-50"
                placeholder="seu@email.com"
              />
            </div>
            <div className="mb-5">
              <label className="block text-[10px] font-mono text-zinc-500 uppercase tracking-wider mb-1.5">Senha</label>
              <input
                type="password"
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-emerald-500/10 hover:bg-emerald-500/20 disabled:opacity-40 disabled:cursor-not-allowed border border-emerald-500/30 hover:border-emerald-500/50 rounded-xl text-sm font-mono text-emerald-300 transition-all"
            >
...
function CarteiraTable({ ativos, quotes, onEdit, onRemove, editingIdx, editBuf, setEditBuf, onSaveEdit, ocultar, usdBrl }) {
  const masked = '••••••'
  ...
  const rawRows = ativos.map((item, originalIdx) => {
    ...
    const isUSD  = item.pais === 'US'
    const fx     = isUSD && usdBrl ? usdBrl : 1
    // custo sempre em BRL (custo medio USD × fx)
    const custo  = item.quantidade * item.custoMedio * fx
    const cotRaw = q.price ?? null
    // liquidação em BRL
    const liq    = cotRaw != null ? item.quantidade * cotRaw * (isUSD ? fx : 1) : null
    const res    = liq != null ? liq - custo : null
    const pct    = res != null && custo > 0 ? (res / custo) * 100 : null
    return { ...item, custo, cot: cotRaw, cotBRL: cotRaw != null ? cotRaw * (isUSD ? fx : 1) : null, varDiaria: q.change ?? null, liq, res, pct, originalIdx, isUSD, fx }
  })
                ...
                <tr key={row.ticker} className={`border-b border-zinc-800/50 transition ${isEditing ? 'bg-blue-500/5' : 'hover:bg-zinc-800/30'}`}>
                  ...
                  <td className="px-3 py-2.5 font-mono font-bold text-zinc-100">
                    <div className="flex items-center gap-1.5">
                      {row.isUSD && <span title="Mercado EUA · USD" className="text-[12px]">🇺🇸</span>}
                      {row.pais === 'BR' && <span title="Mercado BR · BRL" className="text-[12px]">🇧🇷</span>}
                      {row.ticker}
                    </div>
                    {row.isUSD && row.fx > 0 && (
                      <div className="text-[9px] font-mono text-zinc-600 mt-0.5">USD × {row.fx.toFixed(2)}</div>
                    )}
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-right font-mono text-zinc-300">
                    {isEditing
                      ...
                      : ocultar ? masked : (
                        <div>
                          <div>{row.isUSD ? `$ ${row.custoMedio.toFixed(2)}` : fmtMoneyFull(row.custoMedio)}</div>
                          {row.isUSD && <div className="text-[10px] text-zinc-600">USD</div>}
                        </div>
                      )}
                  </td>
                  <td className="px-3 py-2.5 text-right font-mono text-zinc-400">
                    {ocultar ? masked : fmtMoneyFull(row.custo)}
                    {row.isUSD && <div className="text-[10px] text-zinc-600">em BRL</div>}
                  </td>
                  <td className="px-3 py-2.5 text-right font-mono text-zinc-200">
                    {row.cot != null ? (
                      <div>
                        <div>{row.isUSD ? `$ ${row.cot.toFixed(2)}` : fmtMoneyFull(row.cot)}</div>
                        {row.isUSD && row.cotBRL != null && (
                          <div className="text-[10px] text-zinc-600">{fmtMoneyFull(row.cotBRL)}</div>
                        )}
                      </div>
...
function VisaoGeral({ carteiras, quotes, onNavigate, ocultar, user }) {
  const [opcoesPainel, setOpcoesPainel] = React.useState(null)
  React.useEffect(() => {
    if (!user?.uid) return
    Promise.all([
      import('./lib/firebase'),
      import('./lib/optionsCalc')
    ]).then(([fb, calc]) => {
      return fb.loadOptionsData(user.uid).then(data => {
        if (data) setOpcoesPainel(calc.computePainel(data.operacoes || [], data.aporte))
      })
    }).catch(err => console.warn('Opções não carregadas na Visão Geral:', err))
  }, [user?.uid])
  ...
  const opcoesLiquidacao = opcoesPainel?.liquidacaoAberta || 0
  const patrimonioTotal = grandLiq + outrosTotal + opcoesLiquidacao
      ...
      <div className="relative rounded-2xl overflow-hidden border border-zinc-700/50"
        ...
        <div className="relative px-8 py-7">
          ...
          <div className="flex flex-wrap items-end gap-8">
            ...
            {/* Opções (mercado — liquidação das abertas) */}
            {opcoesLiquidacao > 0 && (
              <>
                <div className="w-px h-12 bg-zinc-800 hidden md:block" />
                <div>
                  <div className="text-[10px] font-mono uppercase tracking-wider text-zinc-500 mb-1">Opções (mercado)</div>
                  <div className="text-xl font-mono text-amber-400">{ocultar ? masked : fmtMoneyFull(opcoesLiquidacao)}</div>
                </div>
              </>
            )}
          </div>
...
function PortfolioPage({ user }) {
  ...
  const fetchUsdBrl = async () => {
    try {
      ...
      const cur = d?.USDBRL
      if (cur?.bid) setUsdBrl(parseFloat(cur.bid))
    } catch {}
  }
  ...
  const addItem = () => {
    ...
    updateAtivos(activeTab, ativos => [...ativos, { ticker: t, quantidade: 0, custoMedio: 0, pais: newAtivoPais }])
    setNewTicker('')
    setNewAtivoPais('BR')
    setShowAdd(false)
  ...
  const saveEditOutro = (carteiraId, outroId) => {
    ...
    const agora = new Date().toISOString()
    setCarteiras(prev => prev.map(c => c.id === carteiraId
      ? { ...c, outros: (c.outros || []).map(o => {
          if (o.id !== outroId) return o
          const hist = [...(o.historico || []), { data: agora, valor }]
          return { ...o, quantidade: qtd, valorAtual: valor, atualizadoEm: agora, historico: hist }
        }) }
      : c))
  ...
  return (
    <div className="flex-1 overflow-auto" style={{ background: '#111113' }}>
      ...
      <div className="border-b border-zinc-800 bg-zinc-950 sticky top-0 z-30">
        <div className="px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {activeTab !== 'geral' && activeTab !== 'opcoes' && activeCarteira?.tipo !== 'outros' && (
        ...
        <div className="px-6 flex items-center gap-1 border-t border-zinc-900 overflow-x-auto">
          ...
          {[...carteiras].sort((a, b) => (a.tipo === 'outros' ? 1 : 0) - (b.tipo === 'outros' ? 1 : 0)).map(c => (
            <div key={c.id}
          ...
          <button onClick={() => { setActiveTab('opcoes'); setEditingIdx(null) }}
            className={`px-4 py-2.5 text-[11px] font-mono uppercase tracking-wider border-b-2 transition whitespace-nowrap ${activeTab === 'opcoes' ? 'border-amber-500 text-amber-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}>
            Opções
          </button>
        </div>
      </div>
      ...
      {/* ADD TICKER */}
      {showAdd && activeTab !== 'geral' && activeTab !== 'opcoes' && (
        <div className="px-6 py-3 border-b border-zinc-800 bg-zinc-900/50 flex items-center gap-3 flex-wrap">
          ...
          <input autoFocus type="text" placeholder="Ex: PETR4 ou VOO" value={newTicker}
          ...
          <div className="flex items-center gap-1">
            <span className="text-[10px] font-mono text-zinc-500 mr-1">Mercado:</span>
            <button onClick={() => setNewAtivoPais('BR')}
              className={`px-2 py-1 text-[13px] rounded border transition ${newAtivoPais === 'BR' ? 'border-emerald-500/50 bg-emerald-500/10' : 'border-zinc-700 hover:border-zinc-500'}`}
              title="Brasil">🇧🇷</button>
            <button onClick={() => setNewAtivoPais('US')}
              className={`px-2 py-1 text-[13px] rounded border transition ${newAtivoPais === 'US' ? 'border-blue-500/50 bg-blue-500/10' : 'border-zinc-700 hover:border-zinc-500'}`}
              title="Estados Unidos">🇺🇸</button>
          </div>
          {newAtivoPais === 'US' && (
            <span className="text-[10px] font-mono text-blue-400">Custo médio em USD · liquidação em BRL</span>
          )}
          ...
          <button onClick={() => { setShowAdd(false); setNewAtivoPais('BR') }} className="text-[10px] font-mono text-zinc-500 hover:text-zinc-300 transition">Cancelar</button>
        </div>
      )}
      ...
      <div className="px-6 py-6">
        {activeTab === 'geral'
          ? <VisaoGeral carteiras={carteiras} quotes={quotes} onNavigate={setActiveTab} ocultar={ocultar} user={user} />
          : activeTab === 'opcoes'
          ? <OptionsTab uid={user?.uid} />
  ...
  const fetchAllQuotes = async (isRefresh = false) => {
    ...
    try {
      ...
      await Promise.all(ativos.map(async a => {
        try {
          const url = `https://brapi.dev/api/quote/${a.ticker}?range=1mo&interval=1d&token=${BRAPI_TOKEN}`
